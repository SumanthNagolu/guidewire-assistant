...
if (!process.env.OPENAI_API_KEY) {
  logger.error('[AI Mentor] OPENAI_API_KEY environment variable is not set!');
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      logger.error('[AI Mentor] OPENAI_API_KEY is not configured');
      return jsonError('AI service is not configured. Please contact support.', 503);
    }
    const body = await req.json().catch(() => null);
    logger.debug('[AI Mentor] Request payload received');
    const parsed = mentorRequestSchema.safeParse(body);
    if (!parsed.success) {
      const firstIssue = parsed.error.issues[0]?.message ?? 'Invalid request payload';
      logger.error('[AI Mentor] Validation error:', parsed.error.issues);
    ...
    if (profileResult.error) {
      logger.error('Failed to load user profile:', profileResult.error);
    }
    ...
    if (topicId) {
      const topicQuery = await db
        ...
        .single();
      if (topicQuery.error) {
        logger.error('Failed to load topic for mentor prompt:', topicQuery.error);
...
Respond in a supportive, encouraging tone that promotes active learning.`;
    ...
    if (userMessageError || !userMessageRow) {
      logger.error('Failed to record user mentor message:', userMessageError);
    ...
    const persistAssistantMessage = async () => {
      ...
      const { error: assistantError } = await db
        ...
        .single();
      if (assistantError) {
        logger.error('Failed to record mentor assistant message:', assistantError);
      }
      if (usage?.prompt_tokens && userMessageRow?.id) {
        const existingMetadata = isRecord(userMessageRow.metadata)
          ? (userMessageRow.metadata as Record<string, unknown>)
          : {};
        ...
        if (updateError) {
          logger.error('Failed to update mentor user message metadata:', updateError);
        }
      }
    };
