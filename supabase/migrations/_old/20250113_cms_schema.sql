...
CREATE INDEX idx_blog_versions_post ON blog_post_versions(post_id);
CREATE INDEX idx_blog_versions_created ON blog_post_versions(created_at DESC);
...
CREATE INDEX idx_resources_industry ON resources USING GIN(industry);
...
CREATE INDEX idx_cms_pages_parent ON cms_pages(parent_slug);
CREATE INDEX idx_cms_pages_status ON cms_pages(status);
CREATE INDEX idx_cms_pages_nav ON cms_pages(show_in_nav, nav_order);
...
CREATE INDEX idx_banner_analytics_type ON banner_analytics(event_type);
CREATE INDEX idx_banner_analytics_date ON banner_analytics(created_at DESC);
...
CREATE INDEX idx_audit_entity ON cms_audit_log(entity_type, entity_id);
CREATE INDEX idx_audit_action ON cms_audit_log(action);
CREATE INDEX idx_audit_date ON cms_audit_log(created_at DESC);
...
BEGIN
  ...
  IF OLD.content IS DISTINCT FROM NEW.content OR OLD.title IS DISTINCT FROM NEW.title THEN
    INSERT INTO blog_post_versions (
      ...
      NEW.id, NEW.version, NEW.title, NEW.content, NEW.excerpt, NEW.updated_at
    );
...
CREATE INDEX idx_blog_posts_list ON blog_posts(status, published_at DESC) WHERE status = 'published';
CREATE INDEX idx_resources_list ON resources(status, published_at DESC) WHERE status = 'published';
CREATE INDEX idx_banners_active_list ON banners(is_active, status, display_order) WHERE is_active = true AND status = 'active';
CREATE INDEX idx_cms_pages_navigation ON cms_pages(status, show_in_nav, nav_order) WHERE status = 'published' AND show_in_nav = true;
