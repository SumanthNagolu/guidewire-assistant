-- Import 4 ClaimCenter Quizzes (FIXED for actual schema)
-- Generated: 2025-11-06
-- Topics: cc-01-001 through cc-01-004
-- Total: 20 questions across 4 quizzes

-- Schema-aligned version:
--   quizzes: passing_percentage, is_active (not passing_score, published, time_limit_minutes)
--   quiz_questions: question_text, topic_id, question_type (not question, position)
...
DECLARE
  ...
  v_topic_id UUID;
BEGIN
  SELECT id INTO v_topic_id FROM topics WHERE code = 'cc-01-001';

  -- Create quiz
  INSERT INTO quizzes (
    id, topic_id, title, description, passing_percentage, is_active
  ) VALUES (
    gen_random_uuid(),
    v_topic_id,
...
DECLARE
  ...
  v_topic_id UUID;
BEGIN
  SELECT id INTO v_topic_id FROM topics WHERE code = 'cc-01-002';
...
DECLARE
  ...
  v_topic_id UUID;
BEGIN
  SELECT id INTO v_topic_id FROM topics WHERE code = 'cc-01-003';
...
DECLARE
  ...
  v_topic_id UUID;
BEGIN
  SELECT id INTO v_topic_id FROM topics WHERE code = 'cc-01-004';
